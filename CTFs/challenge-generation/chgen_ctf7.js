#!/usr/bin/env node
/**
 * chgen_ctf7.js - Challenge Generator for CTF7 NorthSide Notes (Insecure Deserialization)
 *
 * Generates per-player flags and credentials, writes them to:
 *   - CTFs/CTF_7_notes_app/src/data/flags.json
 *   - CTFs/CTF_7_notes_app/src/data/users.json
 *   - CTFs/CTF_7_notes_app/src/data/flag-files/<username>.txt
 *
 * Usage:
 *   node chgen_ctf7.js abcd12 efgh34 ijkl56    # Specify player usernames
 *   node chgen_ctf7.js --count 10               # Generate 10 random player usernames
 */

const fs = require('fs')
const path = require('path')
const crypto = require('crypto')
const ctf7Generator = require('./generators/ctf7_generator')

const CTF_DIR = path.resolve(__dirname, '..', 'CTF_7_notes_app')
...
function generateFlag(username) {
  const token = ctf7Generator(username)
  return `${FLAG_PREFIX}{${token}_${username}}`
}
...
function main() {
  ...
  if (countIndex !== -1 && args[countIndex + 1]) {
    ...
    console.error('Usage:')
    console.error('  node chgen_ctf7.js abcd12 efgh34 ijkl56    # Specify player usernames')
    console.error('  node chgen_ctf7.js --count 10               # Generate 10 random players')
    process.exit(1)
  }
  ...
  // Write users.json
  fs.writeFileSync(USERS_OUTPUT, JSON.stringify(credentials, null, 2))
  console.log(`Wrote credentials to ${USERS_OUTPUT}`)

  // Write per-user flag files
  for (const [username, flag] of Object.entries(flags)) {
    const flagFilePath = path.join(FLAG_FILES_DIR, `${username}.txt`)
    fs.writeFileSync(flagFilePath, flag)
    console.log(`Wrote flag file: ${flagFilePath}`)
  }
