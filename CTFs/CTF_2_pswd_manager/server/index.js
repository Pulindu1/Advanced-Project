...
function createApp(options = {}) {
  ...
  const JWT_SECRET = options.jwtSecret || process.env.JWT_SECRET || 'dev-secret-change-me'
  const IS_DEV = options.isDev !== undefined
    ? options.isDev
    : (process.env.NODE_ENV !== 'production' || process.env.CTF_DEV === 'true')
  const FLAGS_PATH = options.flagsPath || process.env.FLAGS_PATH || '/app/flags.json'
  const CREDS_PATH = options.credsPath || process.env.CREDS_PATH || '/app/credentials.json'
  const DATA_DIR = options.dataDir || path.resolve(__dirname, 'data')
  const POW_DIFFICULTY = options.powDifficulty !== undefined ? options.powDifficulty : 4
  const LOCKOUT_THRESHOLD = options.lockoutThreshold !== undefined ? options.lockoutThreshold : 4
  const LOCKOUT_MS = options.lockoutMs !== undefined ? options.lockoutMs : 45 * 1000
  ...
  app.post('/api/auth/login', async (req, res) => {
    ...
    const recordFailure = () => {
      ...
      if (la.count >= LOCKOUT_THRESHOLD) {
        la.lockedUntil = Date.now() + LOCKOUT_MS
        la.count = 0
  ...
  return { app, seedFromFiles, PORT, JWT_SECRET }
}

module.exports = { createApp }

if (require.main === module) {
  const { app, seedFromFiles, PORT } = createApp()
